...
dict <- darktriad %>% var_label() %>% as_data_frame() %>% tidyr::gather(variable, label) %>% knitr::kable(format = "markdown") %>% as.character() %>% paste0(collapse = "\n")
```
...
attributes(darktriad)$distribution = list(
