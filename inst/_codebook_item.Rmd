...
if ( go_vertical ) {
  ...
  # categorical items are plotted vertically because we can use the screen real estate better this way
	...
	if (is.null(choices) || dplyr::n_distinct(na.omit(item)) > length(non_missing_choices)) {
		choices = unique(item)
		names(choices) = choices
	}
	new_height = 2 + 1 * length(non_missing_choices)
...
```{r summary}
item_df <- data.frame(haven::zap_labels(item))
colnames(item_df) = item_name
skimr::skim(item_df)
```
