...
for (i in seq_along(reliabilities)) {
  rel = reliabilities[[i]]
  if (methods::is(rel, "alpha")) {
    cat(knit_print.alpha(rel, paste0(indent, "####")))
  } else {
    cat("\n\n\n```\n")
    psych::print.psych(rel)
    cat("\n```\n\n\n")
  }
}
```
...
```
```{r likert}
likert_plot = likert_from_items(dplyr::select(results, rlang::UQS(rlang::quos(scale_info$scale_item_names))))
