...
test_that("Internal consistencies can be computed", {
  ...
  expect_equal(round(rels$BFIK_consc$internal_consistency$total$std.alpha,3), 0.787)
})

test_that("Retest reliabilities can be computed", {
  data("bfi", package = "codebook")
  library(dplyr)
  expect_warning(bfi2 <- bind_rows(bfi, bfi %>% mutate(created = created + lubridate::years(1))))
  ...
  expect_identical(names(rels$BFIK_agree), c("internal_consistency_T1",
                                             ...
                                             "retest_reliability"))
  expect_equal(round(rels$BFIK_consc$internal_consistency_T1$total$std.alpha,3), 0.787)
  expect_equal(round(rels$BFIK_consc$internal_consistency_T2$total$std.alpha,3), 0.787)
